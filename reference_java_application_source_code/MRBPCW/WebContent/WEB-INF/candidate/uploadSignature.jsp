...
<script type="text/javascript">
	...
	function calculate(input, method) {
		...
		calculateMD5Hash(file, bufferSize, method).then(
			...
			function(progress) {
				...
				if (met == "uploadImage") {
					document.getElementById("attachmentSignatureHash").value = hashVal;
				}
				document.register.action = "CandidateAction_insertCandidateSignature.action";
				enableLoadingAnimation();
	...
	function uploadImage() {
		...
		if (attachmentPhoto == "") {
			...
			if (lowrCaseExtn == "jpg" || lowrCaseExtn == "png" || lowrCaseExtn == "jpeg") {
				...
				var bufferSize = Math.pow(1024, 2) * 0.03; // 30 KB
				var bufferSizeMin = Math.pow(1024, 2) * 0.01; // 10 KB
				var fileSize = $("#attachmentPhoto")[0].files[0].size; // size of the uploaded file
				...
				if (bufferSizeNum < fileSizeNum) {
					message = "Signature size exceeds 30 KB";
					var ulID = "error-ulUploadForm";
					var divID = "error-massageUploadForm";
					createErrorList(message, ulID, divID);
					$('html, body').animate({
					...
					message = "Minimum signature size should be 10 KB";
					var ulID = "error-ulUploadForm";
					var divID = "error-massageUploadForm";
					createErrorList(message, ulID, divID);
					$('html, body').animate({
		...
		try {
			...
			if (responseDiv != "") {
				...
				if (responseArr[0] == "0") {
					...
					$("#imageTd").fadeOut(0);
					var imageHtml = '<img width="200" height="150" src="SignatureImage.jpg?t=' + timestamp + '" name="inputStreamForImage" id="inputStreamForImage"/>';
	...
	function checkuploaded() {
		$.ajax({
			...
			url : "CandidateAction_checkSignExist.action",
			async : true,
			type : 'POST',
	...
	<s:form method="post" name="register" action="CandidateAction"
		...
		<div class="padding_leftright">
			<div class="container common_dashboard">
				<div class="container">
					<s:actionmessage escape="false" cssClass="msgg" id="actionMsgSign" />
					<div id="error-massageUploadForm" style="display: none"
						class="error-massage">
						<div class="error-massage-text"
							style="margin: 0; margin-left: -12px; padding: 0;">
							<ul style="margin: 1; margin-left: 22px; padding: 2;"
								id="error-ulUploadForm">
							</ul>
				...
				<s:hidden name="isDataFound"
					value='<s:property value="imageUploaded"/>' />
				<div style="display: block;">
					<div class="row uploadTable">
						...
						<div class="col-sm-3 col-xs-12 text-center">
							<div class="row">
								<div class="col-sm-12">
									<div class="form-group">
										<label class="control-label">Signature Image <s:text
												name="uploadPhoto.Signimage" /></label>
									</div>
								</div>
							</div>
							...
							<div class="row" id="imageTd">
								<div class="col-sm-12" style="min-height: 90px;">
									<img src="SignatureImage.jpg" height="150" width="100%"
										class="img-responsive" border="1" name="inputStreamForImage"
										id="inputStreamForImage"
										onerror="onErrorMessage('inputStreamForImage')" />
								</div>
								<div class="col-sm-12 mt10">
									<s:file name="attachmentSignature" id="attachmentPhoto"
										label="Attachment Signature " />
									<s:hidden name="attachmentSignatureHash"
										id="attachmentSignatureHash" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mt10">
						...
						<div class="col-sm-9 mt10 col-xs-12">
							<p class="note">
								Click on Choose file and select sign image file to be uploaded
								from your machine. After selecting your Signature, please click
								on Upload button.
								<s:text name="sigNote1" />
							</p>
						</div>
					...
					<div class="row">
						<div class="col-sm-12 mt10">
							...
							<ul class="note ml20">
								<li>The minimum File Size is <%=ConfigurationConstants.getInstance().getPropertyVal(GenericConstants.MIN_SIGN_SIZE_IN_KB)%>
									KB, Maximum File Size is <%=ConfigurationConstants.getInstance().getPropertyVal(GenericConstants.MAX_SIGNATURE_SIZE_IN_KB)%>
									KB and <%=ConfigurationConstants.getInstance().getPropertyVal(GenericConstants.SIGNATURE_TYPE)
						.toLowerCase()%> file images are accepted. <s:text name="sigNote2" /></li>
								<li>Kindly upload clear signature image, if signature image
									upload is not clear then your application is liable to
									rejection by board. <s:text name="sigNote3" />
								</li>
							</ul>
						</div>
					</div>
					<s:hidden name="auditFlag" id="auditFlag" value="Upload Signature"></s:hidden>
				</div>
				<s:token />
			</div>
		</div>
			...
			<div class="container">
				<div class="row">
					<div
						...
						<s:if test='%{signatureMandatory}'>
							<s:if test="%{imageUploaded}">
								<input type="button" onclick="checkuploaded();"
									value="Save & Continue" class="btn btn-warning btn-block" />
							</s:if>
							<s:else>
