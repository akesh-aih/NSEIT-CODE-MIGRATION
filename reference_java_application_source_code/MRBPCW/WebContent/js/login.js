...
function dateVal(form) {
	...
	if (form.value != '') {
		if (regs = form.value.match(re)) {
			...
			/*
			 * year value between 1902 and 2011 if(regs[3] < 1902 || regs[3] > (new Date()).getFullYear()) { alert("Invalid value for year: " + regs[3] + " - must be between 1902
			 * and " + (new Date()).getFullYear()); form.focus(); return false; }
			 */
		} else {
			return false;
		}
...
function registerVal() {
	...
	/*
	 * if(mobileVal(fnameVal.mobileNum2.value) == false) { document.getElementById("mobileID2").style.visibility="visible"; return false; } if(mobileVal(fnameVal.mobileNum2.value) !=
	 * false) { document.getElementById("mobileID2").style.visibility="hidden"; }
	 */
...
/**
 * To be Replaced by javascript from JSP function changeTabs(tabID,divID) { for(i=1; i<=5; i++) { try { document.getElementById("tab"+i).className=""; } catch(e){} } for(i=1; i<=5;
 * i++) { try { document.getElementById("TabDiv"+i).style.display="none"; } catch(e){} } document.getElementById(tabID).className="active";
...
function nameValidator(e) {
	...
	if (unicode != 8) {
		// alert(unicode);
		if ((unicode > 96 && unicode < 123) || (unicode > 64 && unicode < 91) || unicode == 9 || unicode == 190 || unicode == 192 || unicode == 32 || unicode == 39) // removing
																																										// unicode
																																										// == 46
			return true;
		else
...
function educationValidator(e) {
	...
	if ((unicode > 64 && unicode < 91) || (unicode > 96 && unicode < 123) || (unicode > 47 && unicode < 58) || unicode == 45 || unicode == 46 || unicode == 38 || unicode == 8
			|| unicode == 32) {
		return true;
	} else {
		return false;
